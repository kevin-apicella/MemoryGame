...
modoru = pygame.transform.scale(modoru_raw, (int(100*resize_factor), int(100*resize_factor)))
...
while game_running:
    ...
    if game_state == "range: start":
        ...
        for items in card_list:
            items.clicked = False
        player_1 = Players("Player 1", 55+10)
        player_2 = Players("Player 2", 300+10)
        player_3 = Players("Player 3", 550+10)
        player_4 = Players("Player 4", 800+10)
        player_list = [player_1, player_2, player_3, player_4]
        current_player = 0
        ...
        modoru_button = Cards(1566, 6)
        modoru_button.color = (243, 126, 174)
        modoru_button.height = 50*resize_factor
        ...
        for event in ev:
            if event.type == pygame.MOUSEBUTTONDOWN:
                if single.rect.collidepoint(event.pos):
                    ...
                    game_window.blit(cream_surface, (0, 0))
                    generate_players(player_list)
                    modoru_button.draw_card()
                    game_window.blit(modoru, (1566*resize_factor, -14 * resize_factor))
...
# Game state end
        ...
        if len(card_list) > 0:
            if current_player == 0:
                pygame.draw.rect(game_window, RED, (1335*resize_factor, 14*resize_factor, 145*resize_factor, 48*resize_factor), 5)
            elif current_player == 1:
                pygame.draw.rect(game_window, RED, (1335*resize_factor, 258*resize_factor, 148*resize_factor, 48*resize_factor), 5)
            elif current_player == 2:
                pygame.draw.rect(game_window, RED, (1335*resize_factor, 508*resize_factor, 148*resize_factor, 48*resize_factor), 5)
            elif current_player == 3:
                pygame.draw.rect(game_window, RED, (1335*resize_factor, 758*resize_factor, 148*resize_factor, 48*resize_factor), 5)
        for event in ev:
            ...
            # Click a card
            if event.type == pygame.MOUSEBUTTONDOWN:
                if modoru_button.rect.collidepoint(event.pos):
                    game_state = "range: start"
                for item in card_list:
